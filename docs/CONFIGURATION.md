...
面板会在运行时采样并保存流量历史：后台每 5 分钟采样一次，浏览器点击“刷新状态”后满 1 分钟也会补采样；数据保留 31 天。设备详情的 24 小时、7 天、30 天图表显示的是两个样本之间的流量增量；WireGuard 计数器在服务器重启后归零时，面板会从新计数继续记录而不是显示负数。

设备详情可下载 `.conf` 或展示 WireGuard 导入二维码。两者都含该设备的私钥，应和订阅 URL 一样视为密码。
...
| OpenVPN | UDP 1194 | 传统客户端兼容；当前用户的 FLClash 内核不加载此类型，应使用官方 OpenVPN 客户端。 |
